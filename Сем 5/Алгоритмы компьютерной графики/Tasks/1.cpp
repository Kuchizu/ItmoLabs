...
static const struct StarList {
    ...
    { -0.1f, 0, 4, 0.5f, 0.2f },      // 4-лучевая звезда
    { -0.84f, 0.1f, 6, 0.3f, 0.12f }, // 6-лучевая звезда
...
static HRESULT initVertexBuffer(IDirect3DDevice9* pDev, int count, const StarList* list)
{
    int vertexCount = 0;
    for (int i = 0; i < count; i++) {
        vertexCount += list[i].points * 3;
    }

    if (FAILED(pDev->CreateVertexBuffer(vertexCount * sizeof(MY_V3F), 0, D3DFVF_XYZ, D3DPOOL_DEFAULT, &myVertexBuffer, NULL))) {
        return E_FAIL;
    }
    ...
    for (int i = 0, n = 0; i < count; i++) {
        ...
        const float x = list[i].x, y = list[i].y, R = list[i].outerRadius, r = list[i].innerRadius;
        double angle = 0.0;
...
static void CALLBACK OnFrameRender(IDirect3DDevice9* pDev, double time, float elapsedTime, void* userContext)
{
    pDev->Clear(0, NULL, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, D3DXCOLOR(0.5f, 0.0f, 0.0f, 0.5f), 1.0f, 0);
    ...
    if (SUCCEEDED(pDev->BeginScene())) {
        ...
        for (int i = 0; i < myStarCount; i++) {
            pDev->DrawPrimitive(D3DPT_TRIANGLELIST, vertexOffset, myStarList[i].points);
            vertexOffset += myStarList[i].points * 3;
        }
