...
DECLARE_BINOP(divide, DIV)  // <-- добавлено
...
static const char *BINOPS[] = {
        [BIN_PLUS] = "+", [BIN_MINUS] = "-", [BIN_MUL] = "*", [BIN_DIV] = "/"};  // <-- обновлено
...
int main() {
    struct AST *ast1 = add(lit(999), lit(728));   // 999 + 728
    struct AST *ast2 = add(lit(4), mul(lit(2), lit(9))); // 4 + 2 * 9
    struct AST *ast3 = mul(add(lit(3), lit(5)), divide(lit(9), lit(7))); // (3+5) * (9 / 7)
